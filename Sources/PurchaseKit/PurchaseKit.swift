...
public typealias PurchaseFailure = (_ error: Error) -> Void
...
public class PurchaseKit {
    ...
    public func retrieveProductsInfo(productIDs: [String], showIndicator: Bool = false, completion: PurchaseCompletion?, failure: PurchaseFailure? = nil) {
        ...
        SwiftyStoreKit.retrieveProductsInfo(productIDs) { [unowned self] result in
            ...
            if let error = result.error {
                ...
                self.log("Invalid ProductIDs -> \(result.invalidProductIDs)")
    ...
    public func purchaseProduct(_ product: SKProduct, showIndicator: Bool = true, completion: PurchaseCompletion?, failure: PurchaseFailure? = nil) {
        ...
        SwiftyStoreKit.purchaseProduct(product, atomically: atomically) { result in
            ...
            self.handlePurchaseResult( result, atomically: atomically, completion: completion, failure: failure)
        }
    }
    
    public func purchaseProduct(_ productID: String, showIndicator: Bool = true, completion: PurchaseCompletion?, failure: PurchaseFailure? = nil) {
    ...
    public func restorePurchases(atomically: Bool = true, showIndicator: Bool = true, completion: PurchaseCompletion?, failure: PurchaseFailure? = nil) {
        ...
        SwiftyStoreKit.restorePurchases(atomically: atomically) { results in
            ...
            if results.restoreFailedPurchases.count > 0 {
                ...
                if let failure = failure {
                    let error = PurchaseError(message: "Restore Failed")
                    failure(error)
                } else {
                ...
                if let failure = failure {
                    let error = PurchaseError(message: "Nothing to Restore")
                    failure(error)
                } else {
    ...
    private func handlePurchaseResult(_ result: PurchaseResult, atomically: Bool, completion: PurchaseCompletion?, failure: PurchaseFailure?) {
